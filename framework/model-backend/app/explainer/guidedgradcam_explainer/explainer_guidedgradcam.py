...
class GuidedGradCAMExplainer(Explainer):
    ...
    def __init__(self):
        ...
        self.explainer_name = 'Guided GradCAM'
    ...
    def explain(self, model, data, params):
        ...
        # use Guided GradCAM attribute method to get explanation
        layer = 'model.' + params['layer']
        device_ids = params['device_ids']
        interpolate_mode = params['interpolate_mode']
        attribute_to_layer_input = params['attribute_to_layer_input']

        GGC = GuidedGradCam(model, layer=eval(layer), device_ids=device_ids)
        attribution = GGC.attribute(data, target=pred_label_idx, interpolate_mode=interpolate_mode, attribute_to_layer_input=attribute_to_layer_input)
